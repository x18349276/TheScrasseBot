...
if (strpos($text, "/mcd") === 0) {
  ...
  for ($i = 1; $i < count($args); $i++) {
    $response = mcd($response, (int)$args[$i]);
  }
}
if (strpos($text, "/mcm") === 0) {
  ...
  $args = explode(" ", $text);
  $mcd = mcd((int)$args[0], (int)$args[1]);
  $response = (int)$args[0] * (int)$args[1] / $mcd;
  for ($i = 1; $i < count($args); $i++) {
    $mcd = mcd($response, (int)$args[$i]);
    $response = $response * (int)$args[$i] / $mcd;
  }
}
